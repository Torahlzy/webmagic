 ...
 * @since 0.5.1
 */
public class HKBCPic_TopicProcessor implements PageProcessor {
    public static final Logger logger = LoggerFactory.getLogger(HKBCPic_TopicProcessor.class);
    ...
    public void process(Page page) {
        ...
        //主题
        Selectable a_nodes = page.getHtml().xpath("//*[@id=\"moderate\"]/ul/li/h3/a");
        List<String> topics_title = page.getHtml().xpath("//*[@id=\"moderate\"]/ul/li/h3/a/text()").all();
        List<String> topics_url = a_nodes.links().all();
        logger.info("找到sstt主题 {} 个", (topics_url != null ? topics_url.size() : 0));
    ...
    protected void parserTopic(Page page, List<String> topics_title, List<String> topics_url) {
        if (topics_url != null && topics_url.size() > 0) {
            List<HKssttPcTopic> topicsList = new ArrayList<HKssttPcTopic>(topics_url.size());
            for (int i = 0; i < topics_url.size(); i++) {
                HKssttPcTopic hKssxsTopic = HKssttPcTopic.create();
